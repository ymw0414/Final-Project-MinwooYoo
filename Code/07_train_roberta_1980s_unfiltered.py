...
Fine-tune RoBERTa on:
    • 1980s speeches only (file_id = 97–100)
    • UNFILTERED dataset
    • EPOCHS = 1 (fast training)

Settings:
    • batch_size = 32  (same as your fast run)
    • fp16 training (autocast + GradScaler)
...
MODEL_SAVE_DIR = Path("C:/Users/ymw04/Dropbox/shifting_slant/models/roberta_1980s_unfiltered")
...
# ---------------------------------------------------------
# DataLoaders (same as fast version)
# ---------------------------------------------------------
BATCH_SIZE = 32     # <--- the fast version setting
...
NUM_EPOCHS = 1      # <--- same as your fast run
...
# fp16 scaler (same as your fast script)
...
# ---------------------------------------------------------
# Training Loop (same logging as fast version)
# ---------------------------------------------------------
...
for epoch in range(NUM_EPOCHS):
    ...
    for step, batch in enumerate(train_loader):
        ...
        # --- Log every 200 steps (same behavior as fast script)
        if (step + 1) % 200 == 0:
...
metrics_path = EVAL_DIR / "roberta_1980s_unfiltered_metrics.txt"
